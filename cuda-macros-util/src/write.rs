...
pub fn write_fn_header_file<F: FileLike>(of: &mut F, f: &syn::ItemFn, fn_type: FunctionType, fn_hash: &str) -> Result<(), TransError> {
...
pub fn write_fn_cu_file<F: FileLike>(of: &mut F, f: &syn::ItemFn, fn_type: FunctionType, fn_hash: &str) -> Result<(), TransError> {
	writeln!(of, "/* Generated by cuda-macros https://github.com/trolleyman/cuda-macros on {} */", Local::now().to_rfc2822())?;
	writeln!(of, "")?;
...
fn write_fn_decl<F: FileLike>(of: &mut F, f: &syn::ItemFn, fn_type: FunctionType, fn_hash: &str, is_wrapper: bool) -> Result<(), TransError> {
...
fn write_wrapper_fn_body<F: FileLike>(of: &mut F, f: &syn::ItemFn, fn_hash: &str) -> Result<(), TransError> {
	let mut args = vec![];
	for arg in f.decl.inputs.iter() {
