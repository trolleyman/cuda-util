...
pub fn write_fn_source_file<F: FileLike>(of: &mut F, f: &syn::ItemFn, fn_type: FunctionType) -> Result<(), TransError> {
	// Check if file hasn't got header
	if of.seek(SeekFrom::End(0))? == 0 {
...
fn write_cuda_fn_body<F: FileLike>(of: &mut F, stmts: &[syn::Stmt]) -> Result<(), TransError> {
	write_stmts(FileLikeIndent::new(of, 1), stmts, true)
}

fn write_stmts<F: FileLike>(mut of: FileLikeIndent<F>, stmts: &[syn::Stmt], allow_missing_semicolon: bool) -> Result<(), TransError> {
	for (i, stmt) in stmts.iter().enumerate() {
		write_stmt(of.clone(), stmt, allow_missing_semicolon && i == stmts.len() - 1)?;
	}
	Ok(())
}

fn write_stmt<F: FileLike>(mut of: FileLikeIndent<F>, stmt: &syn::Stmt, missing_semicolon_is_return: bool) -> Result<(), TransError> {
	use syn::{Pat, Stmt};
...
fn write_expr_with_brackets<F: FileLike>(mut of: FileLikeIndent<F>, e: &syn::Expr) -> Result<(), TransError> {
...
fn write_expr<F: FileLike>(mut of: FileLikeIndent<F>, e: &syn::Expr) -> Result<(), TransError> {
	...
	match e {
		...
		Expr::Call(e) => {
			if conv::is_item_enabled(&e.attrs, conv::CfgType::DeviceCode)? {
				write_expr_with_brackets(of.clone(), &*e.func)?;
				write!(&mut of, "(")?;
		...
		Expr::MethodCall(e) =>{
			if conv::is_item_enabled(&e.attrs, conv::CfgType::DeviceCode)? {
				...
				write_expr_with_brackets(of.clone(), &*e.receiver)?;
				write!(&mut of, ",")?;
		...
		Expr::Binary(e) => {
			if conv::is_item_enabled(&e.attrs, conv::CfgType::DeviceCode)? {
				write_expr_with_brackets(of.clone(), &*e.left)?;
				write!(&mut of, " ")?;
				write_binop(of.clone(), &e.op)?;
				write!(&mut of, " ")?;
				write_expr_with_brackets(of.clone(), &*e.right)?;
			}
		},
		Expr::Unary(e) => {
			if conv::is_item_enabled(&e.attrs, conv::CfgType::DeviceCode)? {
		...
		Expr::Block(e) => {
			if conv::is_item_enabled(&e.attrs, conv::CfgType::DeviceCode)? {
				writeln!(&mut of, "{{")?;
				write_stmts(of.incr(), &e.block.stmts, false)?;
				writeln!(&mut of, "}}")?;
			}
		},
		Expr::Assign(e) => {
		...
		Expr::Field(e) => {
			if conv::is_item_enabled(&e.attrs, conv::CfgType::DeviceCode)? {
				write_expr_with_brackets(of.clone(), &e.base)?;
				write!(&mut of, ".")?;
				match &e.member {
		...
		Expr::Index(e) => {
			if conv::is_item_enabled(&e.attrs, conv::CfgType::DeviceCode)? {
				write_expr_with_brackets(of.clone(), &e.expr)?;
				write!(&mut of, "[")?;
				write_expr(of.clone(), &e.index)?;
				write!(&mut of, "]")?;
			}
		},
